class AreasController < ApplicationController
  def show
    ...
    @areas = Area.joins(:shops).where(shops: { status: ["無料掲載", "有料掲載"] }).where.not(id: @area.id).distinct
    @shops = @area.shops.where(status: ["無料掲載", "有料掲載"]).includes(:genre)
    @genres = Genre.joins(:shops).where(shops: { area_id: @area.id, status: ["無料掲載", "有料掲載"] }).distinct
