class GenresController < ApplicationController
  def area
    ...
    if controller_name == "districts"
      ...
      @shops = @district.shops.where(genre_id: @genre.id).where(status: ["無料掲載", "有料掲載"])
    elsif controller_name == "areas"
      @area = Area.find(params[:area])
      @district = @area.district
      @shops = @area.shops.where(genre_id: @genre.id).where(status: ["無料掲載", "有料掲載"])
    elsif controller_name == "prefectures"
      @prefecture = Prefecture.find_by(name: params[:area])
      @shops = @prefecture.shops.where(genre_id: @genre.id).where(status: ["無料掲載", "有料掲載"])
    end
  end
end
